...
Feature: OMPGdmUnitMeasurable AEAZ-1980
  ...
  Scenario: Full Load curation
    # 1. test get uom from unit_of_measure_v1 if it exist in cns_plan_unit-localUom (rule T1)
    # 2. reject record if unit_of_measure_v1-uom not exist in cns_plan_unit-localUom (rule T1)
    ...
    Then I check region data "/plan/edm_failed_data" by keyFields "functionalArea,interfaceID,errorCode,sourceSystem,key1,key2,key3,key4,key5"
      ...
      | E1        | SP             | GdmUnitMeasurable | PAL  | CONS_LATAM |      |      |      | Enterprise UOM is missing for local UOM | omp          |
