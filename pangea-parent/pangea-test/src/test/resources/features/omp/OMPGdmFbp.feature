...
#    1.JOIN( cns_fin_plan_qty-localMaterialNumber = material_global_v1-localMaterialNumber AND cns_fin_plan_qty-identifier = 'fbp' ) AND
...
#     JOIN( cns_fin_plan_val-localMaterialNumber = material_global_v1-localMaterialNumber AND cns_fin_plan_val-identifier = 'fbp' ) AND
#    material_global_v1-sourceSystem = cns_plan_parameter-attribute ( where cns_plan_parameter-sourceSystem = material_global_v1-sourceSystem and cns_plan_parameter-dataObject = 'SEND_TO_OMP'  )
...
#    4.get fbpId by T4(Concatenate GDMfbp-ProductID, "-" .  GDMfbp-FromDueDate to create a unique ID.)
