...
public class PlanCnsMaterialPlanStatusDaoImpl extends CommonDaoImpl {
    ...
    public PlanCnsMaterialPlanStatusEntity getEntityWithLocalMaterialNumberAndlLocalPlant(String localMaterialNumber,
        ...
        if (StringUtils.isNotEmpty(localPlant) && StringUtils.isNotEmpty(localMaterialNumber)) {
            String queryString = QueryHelper.buildCriteria(IConstant.CNS_MATERIAL_PLAN_STATUS.LOCAL_MATERIAL_NUMBER)
                    .is(localMaterialNumber).and(IConstant.CNS_MATERIAL_PLAN_STATUS.LOCAL_PLANT).is(localPlant)
                    .toQueryString();
            return queryForObject(IConstant.REGION.CNS_MATERIAL_PLAN_STATUS, queryString,
    ...
    public PlanCnsMaterialPlanStatusEntity getEntityWithConditions(String localMaterialNumber) {
        String queryString = QueryHelper.buildCriteria(IConstant.PLAN_CNS_MATERIAL_PLAN_STATUS.DP_RELEVANT)
                .is(IConstant.VALUE.X).and(IConstant.PLAN_CNS_MATERIAL_PLAN_STATUS.LOCAL_MATERIAL_NUMBER)
                .is(localMaterialNumber).and(IConstant.PLAN_CNS_MATERIAL_PLAN_STATUS.SOURCE_SYSTEM)
                .is(IConstant.VALUE.PROJECT_ONE).toQueryString();
        return queryForObject(IConstant.REGION.PLAN_CNS_MATERIAL_PLAN_STATUS, queryString,
                PlanCnsMaterialPlanStatusEntity.class);
    }
    ...
    public List<PlanCnsMaterialPlanStatusEntity> getEntityListWithConditions(String sourceSystem,
        ...
        if (StringUtils.isNotEmpty(localMaterialNumber) && StringUtils.isNotEmpty(sourceSystem)
            ...
            ADFCriteria adfCriteria = QueryHelper.buildCriteria(IConstant.CNS_MATERIAL_PLAN_STATUS.SOURCE_SYSTEM)
                    .is(sourceSystem).and(IConstant.CNS_MATERIAL_PLAN_STATUS.LOCAL_MATERIAL_NUMBER)
                    .is(localMaterialNumber).and(IConstant.CNS_MATERIAL_PLAN_STATUS.LOCAL_PLANT).is(localPlant);

            if (StringUtils.isNotEmpty(spRelevant) && StringUtils.isNotEmpty(noPlanRelevant)) {
                adfCriteria.and(
                        QueryHelper.buildCriteria(IConstant.PLAN_CNS_MATERIAL_PLAN_STATUS.SP_RELEVANT).is(spRelevant)
                                .or(QueryHelper.buildCriteria(IConstant.PLAN_CNS_MATERIAL_PLAN_STATUS.NO_PLAN_RELEVANT)
                                        .is(noPlanRelevant)));
            } else {
            ...
            return queryForList(IConstant.REGION.PLAN_CNS_MATERIAL_PLAN_STATUS, queryString,
                    PlanCnsMaterialPlanStatusEntity.class);
        }
        return null;
    }
    ...
    public PlanCnsMaterialPlanStatusEntity getEntityWithSPNoPlanRelevantAndLocalMaterialnumber(
        ...
        ADFCriteria adfCriteria = QueryHelper.buildCriteria(IConstant.CNS_MATERIAL_PLAN_STATUS.LOCAL_MATERIAL_NUMBER)
                .is(localMaterialnumber);
        adfCriteria.and(
                QueryHelper.buildCriteria(IConstant.PLAN_CNS_MATERIAL_PLAN_STATUS.SP_RELEVANT).is(IConstant.VALUE.X)
                        .or(QueryHelper.buildCriteria(IConstant.PLAN_CNS_MATERIAL_PLAN_STATUS.NO_PLAN_RELEVANT)
                                .is(IConstant.VALUE.X)));
    ...
    public PlanCnsMaterialPlanStatusEntity getEntityWithLocalMaterialNumberAndSourceSystem(String localMaterialNumber,
        ...
        String queryString = QueryHelper.buildCriteria(IConstant.PLAN_CNS_MATERIAL_PLAN_STATUS.LOCAL_MATERIAL_NUMBER)
                .is(localMaterialNumber).and(IConstant.PLAN_CNS_MATERIAL_PLAN_STATUS.SOURCE_SYSTEM).is(sourceSystem)
                .toQueryString();
        return queryForObject(IConstant.REGION.PLAN_CNS_MATERIAL_PLAN_STATUS, queryString,
                PlanCnsMaterialPlanStatusEntity.class);
    }
    ...
    public PlanCnsMaterialPlanStatusEntity getEntityWithThereConditions(String localMaterialNumber, String localPlant,
        ...
        if (StringUtils.isNotEmpty(localMaterialNumber) && StringUtils.isNotEmpty(localPlant)
            ...
            String queryString = QueryHelper.buildCriteria(IConstant.PLAN_CNS_MATERIAL_PLAN_STATUS.DP_RELEVANT)
                    .is(dpRelevant).and(IConstant.PLAN_CNS_MATERIAL_PLAN_STATUS.LOCAL_MATERIAL_NUMBER)
                    .is(localMaterialNumber).and(IConstant.PLAN_CNS_MATERIAL_PLAN_STATUS.LOCAL_PLANT).is(localPlant)
                    .toQueryString();
            return queryForObject(IConstant.REGION.PLAN_CNS_MATERIAL_PLAN_STATUS, queryString,
                    PlanCnsMaterialPlanStatusEntity.class);
        }
    ...
    public List<PlanCnsMaterialPlanStatusEntity> getEntityWithMaterialNumber(String materialNumber) {
        if (StringUtils.isNotEmpty(materialNumber)) {
            String queryString = QueryHelper.buildCriteria(IConstant.PLAN_CNS_MATERIAL_PLAN_STATUS.MATERIAL_NUMBER)
    ...
    public List<PlanCnsMaterialPlanStatusEntity> getEntityListWithLocalMaterialNumber(String localMaterialNumber) {
        if (StringUtils.isNotEmpty(localMaterialNumber)) {
            String queryString = QueryHelper
                    .buildCriteria(IConstant.PLAN_CNS_MATERIAL_PLAN_STATUS.LOCAL_MATERIAL_NUMBER)
                    .is(localMaterialNumber).toQueryString();
            return queryForList(IConstant.REGION.PLAN_CNS_MATERIAL_PLAN_STATUS, queryString,
                    PlanCnsMaterialPlanStatusEntity.class);
        }
    ...
    public PlanCnsMaterialPlanStatusEntity getEntityWithLocalMaterialNumberSourceSystemAndRelevant(String sourceSystem,
        ...
        if (StringUtils.isNotEmpty(sourceSystem) && StringUtils.isNotEmpty(localMaterialNumber)) {
            String queryString = QueryHelper.buildCriteria(IConstant.CNS_MATERIAL_PLAN_STATUS.SOURCE_SYSTEM)
                    .is(sourceSystem).and(IConstant.CNS_MATERIAL_PLAN_STATUS.LOCAL_MATERIAL_NUMBER)
                    .is(localMaterialNumber).toQueryString();
            PlanCnsMaterialPlanStatusEntity materialPlanStatusEntity = queryForObject(
                    IConstant.REGION.PLAN_CNS_MATERIAL_PLAN_STATUS, queryString, PlanCnsMaterialPlanStatusEntity.class);

            if (null != materialPlanStatusEntity) {
                if (IConstant.VALUE.X.equals(materialPlanStatusEntity.getDpRelevant())
                        || IConstant.VALUE.X.equals(materialPlanStatusEntity.getSpRelevant())
    ...
    public PlanCnsMaterialPlanStatusEntity getEntityWithSourceSystemAndLocalMaterialNumberAndLocalPlant(
        ...
        String queryString = QueryHelper.buildCriteria(IConstant.CNS_MATERIAL_PLAN_STATUS.SOURCE_SYSTEM)
                .is(sourceSystem).and(IConstant.CNS_MATERIAL_PLAN_STATUS.LOCAL_MATERIAL_NUMBER).is(localMaterialNumber)
    ...
    public PlanCnsMaterialPlanStatusEntity getEntityWithLocalParentCode(String localParentCode) {
        if (StringUtils.isNotEmpty(localParentCode)) {
            String queryString = QueryHelper.buildCriteria(IConstant.PLAN_CNS_MATERIAL_PLAN_STATUS.LOCAL_PARENT_CODE)
    ...
    public PlanCnsMaterialPlanStatusEntity getEntityWithLocalParentCodeAndDp(String localParentCode) {
        if (StringUtils.isNotEmpty(localParentCode)) {
            String queryString = QueryHelper.buildCriteria(IConstant.PLAN_CNS_MATERIAL_PLAN_STATUS.LOCAL_PARENT_CODE)
                    .is(localParentCode).and(IConstant.PLAN_CNS_MATERIAL_PLAN_STATUS.DP_RELEVANT).is(IConstant.VALUE.X)
                    .toQueryString();
    ...
    public PlanCnsMaterialPlanStatusEntity getPlanCnsMaterialPlanStatusEntity4251(String sourceSystem,
        ...
        if (StringUtils.isBlank(sourceSystem)) {
            ...
            adfCriteria.and(QueryHelper.buildCriteria(IConstant.PLAN_CNS_MATERIAL_PLAN_STATUS.SOURCE_SYSTEM)
                    .is(sourceSystem.trim()));
        }
        if (StringUtils.isBlank(localMaterialNumber)) {
            adfCriteria.and(
                    QueryHelper.buildCriteria(IConstant.PLAN_CNS_MATERIAL_PLAN_STATUS.LOCAL_MATERIAL_NUMBER).isNull());
        } else {
            adfCriteria.and(QueryHelper.buildCriteria(IConstant.PLAN_CNS_MATERIAL_PLAN_STATUS.LOCAL_MATERIAL_NUMBER)
        ...
        if (StringUtils.isBlank(localPlant)) {
            ...
            adfCriteria.and(QueryHelper.buildCriteria(IConstant.PLAN_CNS_MATERIAL_PLAN_STATUS.LOCAL_PLANT)
                    .is(localPlant.trim()));
        }
        adfCriteria.and(QueryHelper.buildCriteria(IConstant.PLAN_CNS_MATERIAL_PLAN_STATUS.SP_RELEVANT)
                .is(IConstant.PLAN_CNS_MATERIAL_PLAN_STATUS.FIELD_X));
        return queryForObject(IConstant.REGION.PLAN_CNS_MATERIAL_PLAN_STATUS, adfCriteria.toQueryString(),
                PlanCnsMaterialPlanStatusEntity.class);
    }
    ...
    public List<PlanCnsMaterialPlanStatusEntity> getEntitiesWithLocalMaterialNumberLocalPlantSourceSystemAndRelevant(
        ...
        String queryString = QueryHelper.buildCriteria(IConstant.CNS_MATERIAL_PLAN_STATUS.LOCAL_MATERIAL_NUMBER)
                .is(localMaterialNumber).and(IConstant.CNS_MATERIAL_PLAN_STATUS.LOCAL_PLANT).is(localPlantNum)
                .and(IConstant.CNS_MATERIAL_PLAN_STATUS.SOURCE_SYSTEM).is(sourceSystem)
                .and(IConstant.CNS_MATERIAL_PLAN_STATUS.SP_RELEVANT).is("X").toQueryString();
        return queryForList(IConstant.REGION.PLAN_CNS_MATERIAL_PLAN_STATUS, queryString,
                PlanCnsMaterialPlanStatusEntity.class);
    }
