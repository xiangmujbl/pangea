package com.jnj.pangea.common.entity.project_one;

import com.jnj.pangea.common.entity.CommonEntity;

import java.util.Map;

public class LikpEntity extends CommonEntity {

    private String ablad;
    private String abssc;
    private String aedat;
    private String aenam;
    private String akkur;
    private String akprz;
    private String akwae;
    private String amtbl;
    private String anzpk;
    private String aulwe;
    private String autlf;
    private String berot;
    private String bev1luleinh;
    private String bev1rpcont;
    private String bev1rpfaess;
    private String bev1rpflgnr;
    private String bev1rpkist;
    private String bev1rpsonst;
    private String bldat;
    private String bolnr;
    private String borgrgrp;
    private String btgew;
    private String bzirk;
    private String cmfre;
    private String cmngv;
    private String cmwae;
    private String commn;
    private String contdg;
    private String ctlpc;
    private String dirta;
    private String erdat;
    private String ernam;
    private String erzet;
    private String exnum;
    private String expkz;
    private String faksk;
    private String fkaiv;
    private String fkarv;
    private String fkdat;
    private String fkdiv;
    private String folar;
    private String gewei;
    private String grulg;
    private String grupp;
    private String handle;
    private String imwrk;
    private String inco1;
    private String inco2;
    private String kalsm;
    private String kalsmch;
    private String kalsp;
    private String kdgrp;
    private String kkber;
    private String klief;
    private String knfak;
    private String knkli;
    private String knump;
    private String knumv;
    private String kodat;
    private String kouhr;
    private String kunag;
    private String kuniv;
    private String kunnr;
    private String kzazu;
    private String kzwab;
    private String lcnum;
    private String lddat;
    private String lduhr;
    private String lfart;
    private String lfdat;
    private String lfuhr;
    private String lgbzo;
    private String lgnum;
    private String lgtor;
    private String lifex;
    private String lifnr;
    private String lifsk;
    private String lispl;
    private String locguid;
    private String lprio;
    private String lstel;
    private String mandt;
    private String msractive;
    private String netwr;
    private String ntgew;
    private String perfk;
    private String pioiv;
    private String podat;
    private String potim;
    private String printerprofile;
    private String proli;
    private String prtnr;
    private String prvbe;
    private String routa;
    private String route;
    private String sakes;
    private String sbgrp;
    private String sdabw;
    private String spaiv;
    private String speaccappsts;
    private String spebillingind;
    private String specarrierind;
    private String spechngsys;
    private String spegeoroute;
    private String spegeorouteind;
    private String spegtsrel;
    private String spegtsrtcde;
    private String speinvbfrgi;
    private String spelescenario;
    private String spelifextype;
    private String spelocseq;
    private String speloekz;
    private String speorigsys;
    private String spepronumber;
    private String speqistatus;
    private String speredind;
    private String spereltmstmp;
    private String speretcanc;
    private String sperevvlstk;
    private String speshpinfsts;
    private String spettype;
    private String speunitsystem;
    private String spewauhrist;
    private String spewazoneist;
    private String stafo;
    private String stcur;
    private String stgelocchange;
    private String stwae;
    private String stzkl;
    private String stzzu;
    private String tcode;
    private String tddat;
    private String tduhr;
    private String ternr;
    private String tmctrlkey;
    private String tpgrp;
    private String tpqua;
    private String tpsid;
    private String tragr;
    private String traid;
    private String traty;
    private String trmtyp;
    private String trspg;
    private String tsegfl;
    private String tsegtp;
    private String tzonis;
    private String tzonrc;
    private String vbeak;
    private String vbeln;
    private String vbtyp;
    private String vbund;
    private String verur;
    private String verursys;
    private String vganz;
    private String vkbur;
    private String vkoiv;
    private String vkorg;
    private String vlstk;
    private String voleh;
    private String volum;
    private String vsart;
    private String vsbed;
    private String vstel;
    private String vtwiv;
    private String wadat;
    private String wadatist;
    private String waerk;
    private String wauhr;
    private String werks;
    private String xabln;
    private String xblnr;
    private String xwoff;
    private String zukrl;
    private String gstrp;

    public LikpEntity(Map<String, Object> map) {
        super(map);

        setAblad((String) map.get("ablad"));
        setAbssc((String) map.get("abssc"));
        setAedat((String) map.get("aedat"));
        setAenam((String) map.get("aenam"));
        setAkkur((String) map.get("akkur"));
        setAkprz((String) map.get("akprz"));
        setAkwae((String) map.get("akwae"));
        setAmtbl((String) map.get("amtbl"));
        setAnzpk((String) map.get("anazpk"));
        setAulwe((String) map.get("aulwe"));
        setBerot((String) map.get("berot"));
        setBev1luleinh((String) map.get("bev1luleinh"));
        setBev1rpcont((String) map.get("bev1rpcont"));
        setBev1rpfaess((String) map.get("bev1rpfaess"));
        setBev1rpflgnr((String) map.get("bev1rpflgnr"));
        setBev1rpkist((String) map.get("bev1rpkist"));
        setBev1rpsonst((String) map.get("bev1rpsonst"));
        setBldat((String) map.get("bldat"));
        setBolnr((String) map.get("bolnr"));
        setBorgrgrp((String) map.get("borgrgrp"));
        setBtgew((String) map.get("btgew"));
        setBzirk((String) map.get("bzirk"));
        setCmfre((String) map.get("cmfre"));
        setCmngv((String) map.get("cmngv"));
        setCmwae((String) map.get("cmwae"));
        setCommn((String) map.get("commn"));
        setContdg((String) map.get("contdg"));
        setCtlpc((String) map.get("ctlpc"));
        setDirta((String) map.get("dirta"));
        setErdat((String) map.get("erdat"));
        setErnam((String) map.get("ernam"));
        setErzet((String) map.get("erzet"));
        setExnum((String) map.get("exnum"));
        setExpkz((String) map.get("expkz"));
        setFaksk((String) map.get("faksk"));
        setFkaiv((String) map.get("fkaiv"));
        setFkarv((String) map.get("fkarv"));
        setFkdat((String) map.get("fkdat"));
        setFkdiv((String) map.get("fkdiv"));
        setFolar((String) map.get("folar"));
        setGewei((String) map.get("gewei"));
        setGrulg((String) map.get("grulg"));
        setGrupp((String) map.get("grupp"));
        setGstrp((String) map.get("gstrp"));
        setHandle((String) map.get("handle"));
        setImwrk((String) map.get("imwrk"));
        setInco1((String) map.get("inco1"));
        setInco2((String) map.get("inco2"));
        setKalsm((String) map.get("kalsm"));
        setKalsmch((String) map.get("kalsmch"));
        setKalsp((String) map.get("kalsp"));
        setKdgrp((String) map.get("kdgrp"));
        setKkber((String) map.get("kkber"));
        setKlief((String) map.get("klief"));
        setKnfak((String) map.get("knfak"));
        setKnkli((String) map.get("knkli"));
        setKnump((String) map.get("knump"));
        setKnumv((String) map.get("knumv"));
        setKodat((String) map.get("kodat"));
        setKouhr((String) map.get("kouhr"));
        setKunag((String) map.get("kunag"));
        setKuniv((String) map.get("kuniv"));
        setKunnr((String) map.get("kunnr"));
        setKzazu((String) map.get("kzazu"));
        setKzwab((String) map.get("kzwab"));
        setLcnum((String) map.get("lenum"));
        setLddat((String) map.get("lddat"));
        setLduhr((String) map.get("lduhr"));
        setLfart((String) map.get("lfart"));
        setLfdat((String) map.get("lfdat"));
        setLfuhr((String) map.get("lfuhr"));
        setLgbzo((String) map.get("lgbzo"));
        setLgnum((String) map.get("lgnum"));
        setLgtor((String) map.get("lgtor"));
        setLifex((String) map.get("lifex"));
        setLifnr((String) map.get("lifnr"));
        setLifsk((String) map.get("lifsk"));
        setLispl((String) map.get("lispl"));
        setLocguid((String) map.get("locguid"));
        setLprio((String) map.get("lprio"));
        setLstel((String) map.get("lstel"));
        setMandt((String) map.get("mandt"));
        setMsractive((String) map.get("msractive"));
        setNetwr((String) map.get("netwr"));
        setNtgew((String) map.get("ntgew"));
        setPerfk((String) map.get("perfk"));
        setPioiv((String) map.get("pioiv"));
        setPodat((String) map.get("podat"));
        setPotim((String) map.get("potim"));
        setPrinterprofile((String) map.get("printerprofile"));
        setProli((String) map.get("proli"));
        setPrtnr((String) map.get("prtnr"));
        setPrvbe((String) map.get("prvbe"));
        setRouta((String) map.get("routa"));
        setRoute((String) map.get("route"));
        setSakes((String) map.get("sakes"));
        setSbgrp((String) map.get("sbgrp"));
        setSdabw((String) map.get("sdabw"));
        setSpaiv((String) map.get("spaiv"));
        setSpeaccappsts((String) map.get("speaccappsts"));
        setSpebillingind((String) map.get("spebillingind"));
        setSpecarrierind((String) map.get("specarrierind"));
        setSpechngsys((String) map.get("spechngsys"));
        setSpegeoroute((String) map.get("spegeoroute"));
        setSpegeorouteind((String) map.get("spegeorouteind"));
        setSpegtsrel((String) map.get("spegtsrel"));
        setSpegtsrtcde((String) map.get("spegtsrtcde"));
        setSpeinvbfrgi((String) map.get("speinvbfrgi"));
        setSpelescenario((String) map.get("spelescenari"));
        setSpelifextype((String) map.get("spelifextype"));
        setSpelocseq((String) map.get("spelocseq"));
        setSpeloekz((String) map.get("speloekz"));
        setSpeorigsys((String) map.get("speorigsys"));
        setSpepronumber((String) map.get("spepronumber"));
        setSpeqistatus((String) map.get("speqistatus"));
        setSperedind((String) map.get("speredind"));
        setSpereltmstmp((String) map.get("spereltmstmp"));
        setSperetcanc((String) map.get("speretcanc"));
        setSperevvlstk((String) map.get("sperevvlstk"));
        setSpeshpinfsts((String) map.get("speshpinfsts"));
        setSpettype((String) map.get("spettype"));
        setSpeunitsystem((String) map.get("speunitsystem"));
        setSpewauhrist((String) map.get("spewauhrist"));
        setSpewazoneist((String) map.get("spewazoneist"));
        setStafo((String) map.get("stafo"));
        setStcur((String) map.get("stcur"));
        setStgelocchange((String) map.get("stgelocchange"));
        setStwae((String) map.get("stwae"));
        setStzkl((String) map.get("stzkl"));
        setStzzu((String) map.get("stzzu"));
        setTcode((String) map.get("tcode"));
        setTddat((String) map.get("tddat"));
        setTduhr((String) map.get("tduhr"));
        setTernr((String) map.get("ternr"));
        setTmctrlkey((String) map.get("tmctrlkey"));
        setTpgrp((String) map.get("tpgrp"));
        setTpqua((String) map.get("tpqua"));
        setTpsid((String) map.get("tpsid"));
        setTragr((String) map.get("tragr"));
        setTraid((String) map.get("traid"));
        setTraty((String) map.get("traty"));
        setTrmtyp((String) map.get("trmtyp"));
        setTrspg((String) map.get("trspg"));
        setTsegfl((String) map.get("tsegfl"));
        setTsegtp((String) map.get("tsegtp"));
        setTzonis((String) map.get("tzonis"));
        setTzonrc((String) map.get("tzonrc"));
        setVbeak((String) map.get("vbeak"));
        setVbeln((String) map.get("vbeln"));
        setVbtyp((String) map.get("vbtyp"));
        setVbund((String) map.get("vbund"));
        setVerur((String) map.get("verur"));
        setVerursys((String) map.get("verursys"));
        setVganz((String) map.get("vganz"));
        setVkbur((String) map.get("vkbur"));
        setVkoiv((String) map.get("vkoiv"));
        setVkorg((String) map.get("vkorg"));
        setVlstk((String) map.get("vlstk"));
        setVoleh((String) map.get("voleh"));
        setVolum((String) map.get("volum"));
        setVsart((String) map.get("vsart"));
        setVsbed((String) map.get("vsbed"));
        setVstel((String) map.get("vstel"));
        setVtwiv((String) map.get("vtwiv"));
        setWadat((String) map.get("wadat"));
        setWadatist((String) map.get("wadatist"));
        setWaerk((String) map.get("waerk"));
        setWauhr((String) map.get("wauhr"));
        setWerks((String) map.get("werks"));
        setXabln((String) map.get("xabln"));
        setXblnr((String) map.get("xblnr"));
        setXwoff((String) map.get("xwoff"));
        setZukrl((String) map.get("xukrl"));
    }

    public String getAblad() {
        return ablad;
    }

    public void setAblad(String ablad) {
        this.ablad = ablad;
    }

    public String getAbssc() {
        return abssc;
    }

    public void setAbssc(String abssc) {
        this.abssc = abssc;
    }

    public String getAedat() {
        return aedat;
    }

    public void setAedat(String aedat) {
        this.aedat = aedat;
    }

    public String getAenam() {
        return aenam;
    }

    public void setAenam(String aenam) {
        this.aenam = aenam;
    }

    public String getAkkur() {
        return akkur;
    }

    public void setAkkur(String akkur) {
        this.akkur = akkur;
    }

    public String getAkprz() {
        return akprz;
    }

    public void setAkprz(String akprz) {
        this.akprz = akprz;
    }

    public String getAkwae() {
        return akwae;
    }

    public void setAkwae(String akwae) {
        this.akwae = akwae;
    }

    public String getAmtbl() {
        return amtbl;
    }

    public void setAmtbl(String amtbl) {
        this.amtbl = amtbl;
    }

    public String getAnzpk() {
        return anzpk;
    }

    public void setAnzpk(String anzpk) {
        this.anzpk = anzpk;
    }

    public String getAulwe() {
        return aulwe;
    }

    public void setAulwe(String aulwe) {
        this.aulwe = aulwe;
    }

    public String getAutlf() {
        return autlf;
    }

    public void setAutlf(String autlf) {
        this.autlf = autlf;
    }

    public String getBerot() {
        return berot;
    }

    public void setBerot(String berot) {
        this.berot = berot;
    }

    public String getBev1luleinh() {
        return bev1luleinh;
    }

    public void setBev1luleinh(String bev1luleinh) {
        this.bev1luleinh = bev1luleinh;
    }

    public String getBev1rpcont() {
        return bev1rpcont;
    }

    public void setBev1rpcont(String bev1rpcont) {
        this.bev1rpcont = bev1rpcont;
    }

    public String getBev1rpfaess() {
        return bev1rpfaess;
    }

    public void setBev1rpfaess(String bev1rpfaess) {
        this.bev1rpfaess = bev1rpfaess;
    }

    public String getBev1rpflgnr() {
        return bev1rpflgnr;
    }

    public void setBev1rpflgnr(String bev1rpflgnr) {
        this.bev1rpflgnr = bev1rpflgnr;
    }

    public String getBev1rpkist() {
        return bev1rpkist;
    }

    public void setBev1rpkist(String bev1rpkist) {
        this.bev1rpkist = bev1rpkist;
    }

    public String getBev1rpsonst() {
        return bev1rpsonst;
    }

    public void setBev1rpsonst(String bev1rpsonst) {
        this.bev1rpsonst = bev1rpsonst;
    }

    public String getBldat() {
        return bldat;
    }

    public void setBldat(String bldat) {
        this.bldat = bldat;
    }

    public String getBorgrgrp() {
        return borgrgrp;
    }

    public void setBorgrgrp(String borgrgrp) {
        this.borgrgrp = borgrgrp;
    }

    public String getBtgew() {
        return btgew;
    }

    public void setBtgew(String btgew) {
        this.btgew = btgew;
    }

    public String getBzirk() {
        return bzirk;
    }

    public void setBzirk(String bzirk) {
        this.bzirk = bzirk;
    }

    public String getCmfre() {
        return cmfre;
    }

    public void setCmfre(String cmfre) {
        this.cmfre = cmfre;
    }

    public String getCmngv() {
        return cmngv;
    }

    public void setCmngv(String cmngv) {
        this.cmngv = cmngv;
    }

    public String getCmwae() {
        return cmwae;
    }

    public void setCmwae(String cmwae) {
        this.cmwae = cmwae;
    }

    public String getCommn() {
        return commn;
    }

    public void setCommn(String commn) {
        this.commn = commn;
    }

    public String getContdg() {
        return contdg;
    }

    public void setContdg(String contdg) {
        this.contdg = contdg;
    }

    public String getCtlpc() {
        return ctlpc;
    }

    public void setCtlpc(String ctlpc) {
        this.ctlpc = ctlpc;
    }

    public String getDirta() {
        return dirta;
    }

    public void setDirta(String dirta) {
        this.dirta = dirta;
    }

    public String getErnam() {
        return ernam;
    }

    public void setErnam(String ernam) {
        this.ernam = ernam;
    }

    public String getErzet() {
        return erzet;
    }

    public void setErzet(String erzet) {
        this.erzet = erzet;
    }

    public String getExnum() {
        return exnum;
    }

    public void setExnum(String exnum) {
        this.exnum = exnum;
    }

    public String getExpkz() {
        return expkz;
    }

    public void setExpkz(String expkz) {
        this.expkz = expkz;
    }

    public String getFaksk() {
        return faksk;
    }

    public void setFaksk(String faksk) {
        this.faksk = faksk;
    }

    public String getFkaiv() {
        return fkaiv;
    }

    public void setFkaiv(String fkaiv) {
        this.fkaiv = fkaiv;
    }

    public String getFkarv() {
        return fkarv;
    }

    public void setFkarv(String fkarv) {
        this.fkarv = fkarv;
    }

    public String getFkdat() {
        return fkdat;
    }

    public void setFkdat(String fkdat) {
        this.fkdat = fkdat;
    }

    public String getFkdiv() {
        return fkdiv;
    }

    public void setFkdiv(String fkdiv) {
        this.fkdiv = fkdiv;
    }

    public String getFolar() {
        return folar;
    }

    public void setFolar(String folar) {
        this.folar = folar;
    }

    public String getGewei() {
        return gewei;
    }

    public void setGewei(String gewei) {
        this.gewei = gewei;
    }

    public String getGrulg() {
        return grulg;
    }

    public void setGrulg(String grulg) {
        this.grulg = grulg;
    }

    public String getGrupp() {
        return grupp;
    }

    public void setGrupp(String grupp) {
        this.grupp = grupp;
    }

    public String getHandle() {
        return handle;
    }

    public void setHandle(String handle) {
        this.handle = handle;
    }

    public String getImwrk() {
        return imwrk;
    }

    public void setImwrk(String imwrk) {
        this.imwrk = imwrk;
    }

    public String getInco1() {
        return inco1;
    }

    public void setInco1(String inco1) {
        this.inco1 = inco1;
    }

    public String getInco2() {
        return inco2;
    }

    public void setInco2(String inco2) {
        this.inco2 = inco2;
    }

    public String getKalsm() {
        return kalsm;
    }

    public void setKalsm(String kalsm) {
        this.kalsm = kalsm;
    }

    public String getKalsmch() {
        return kalsmch;
    }

    public void setKalsmch(String kalsmch) {
        this.kalsmch = kalsmch;
    }

    public String getKalsp() {
        return kalsp;
    }

    public void setKalsp(String kalsp) {
        this.kalsp = kalsp;
    }

    public String getKdgrp() {
        return kdgrp;
    }

    public void setKdgrp(String kdgrp) {
        this.kdgrp = kdgrp;
    }

    public String getKkber() {
        return kkber;
    }

    public void setKkber(String kkber) {
        this.kkber = kkber;
    }

    public String getKlief() {
        return klief;
    }

    public void setKlief(String klief) {
        this.klief = klief;
    }

    public String getKnfak() {
        return knfak;
    }

    public void setKnfak(String knfak) {
        this.knfak = knfak;
    }

    public String getKnkli() {
        return knkli;
    }

    public void setKnkli(String knkli) {
        this.knkli = knkli;
    }

    public String getKnump() {
        return knump;
    }

    public void setKnump(String knump) {
        this.knump = knump;
    }

    public String getKnumv() {
        return knumv;
    }

    public void setKnumv(String knumv) {
        this.knumv = knumv;
    }

    public String getKodat() {
        return kodat;
    }

    public void setKodat(String kodat) {
        this.kodat = kodat;
    }

    public String getKouhr() {
        return kouhr;
    }

    public void setKouhr(String kouhr) {
        this.kouhr = kouhr;
    }

    public String getKunag() {
        return kunag;
    }

    public void setKunag(String kunag) {
        this.kunag = kunag;
    }

    public String getKuniv() {
        return kuniv;
    }

    public void setKuniv(String kuniv) {
        this.kuniv = kuniv;
    }

    public String getKunnr() {
        return kunnr;
    }

    public void setKunnr(String kunnr) {
        this.kunnr = kunnr;
    }

    public String getKzazu() {
        return kzazu;
    }

    public void setKzazu(String kzazu) {
        this.kzazu = kzazu;
    }

    public String getKzwab() {
        return kzwab;
    }

    public void setKzwab(String kzwab) {
        this.kzwab = kzwab;
    }

    public String getLcnum() {
        return lcnum;
    }

    public void setLcnum(String lcnum) {
        this.lcnum = lcnum;
    }

    public String getLddat() {
        return lddat;
    }

    public void setLddat(String lddat) {
        this.lddat = lddat;
    }

    public String getLduhr() {
        return lduhr;
    }

    public void setLduhr(String lduhr) {
        this.lduhr = lduhr;
    }

    public String getLfuhr() {
        return lfuhr;
    }

    public void setLfuhr(String lfuhr) {
        this.lfuhr = lfuhr;
    }

    public String getLgbzo() {
        return lgbzo;
    }

    public void setLgbzo(String lgbzo) {
        this.lgbzo = lgbzo;
    }

    public String getLgnum() {
        return lgnum;
    }

    public void setLgnum(String lgnum) {
        this.lgnum = lgnum;
    }

    public String getLgtor() {
        return lgtor;
    }

    public void setLgtor(String lgtor) {
        this.lgtor = lgtor;
    }

    public String getLifsk() {
        return lifsk;
    }

    public void setLifsk(String lifsk) {
        this.lifsk = lifsk;
    }

    public String getLispl() {
        return lispl;
    }

    public void setLispl(String lispl) {
        this.lispl = lispl;
    }

    public String getLocguid() {
        return locguid;
    }

    public void setLocguid(String locguid) {
        this.locguid = locguid;
    }

    public String getLprio() {
        return lprio;
    }

    public void setLprio(String lprio) {
        this.lprio = lprio;
    }

    public String getLstel() {
        return lstel;
    }

    public void setLstel(String lstel) {
        this.lstel = lstel;
    }

    public String getMandt() {
        return mandt;
    }

    public void setMandt(String mandt) {
        this.mandt = mandt;
    }

    public String getMsractive() {
        return msractive;
    }

    public void setMsractive(String msractive) {
        this.msractive = msractive;
    }

    public String getNetwr() {
        return netwr;
    }

    public void setNetwr(String netwr) {
        this.netwr = netwr;
    }

    public String getNtgew() {
        return ntgew;
    }

    public void setNtgew(String ntgew) {
        this.ntgew = ntgew;
    }

    public String getPerfk() {
        return perfk;
    }

    public void setPerfk(String perfk) {
        this.perfk = perfk;
    }

    public String getPioiv() {
        return pioiv;
    }

    public void setPioiv(String pioiv) {
        this.pioiv = pioiv;
    }

    public String getPodat() {
        return podat;
    }

    public void setPodat(String podat) {
        this.podat = podat;
    }

    public String getPotim() {
        return potim;
    }

    public void setPotim(String potim) {
        this.potim = potim;
    }

    public String getPrinterprofile() {
        return printerprofile;
    }

    public void setPrinterprofile(String printerprofile) {
        this.printerprofile = printerprofile;
    }

    public String getProli() {
        return proli;
    }

    public void setProli(String proli) {
        this.proli = proli;
    }

    public String getPrtnr() {
        return prtnr;
    }

    public void setPrtnr(String prtnr) {
        this.prtnr = prtnr;
    }

    public String getPrvbe() {
        return prvbe;
    }

    public void setPrvbe(String prvbe) {
        this.prvbe = prvbe;
    }

    public String getRouta() {
        return routa;
    }

    public void setRouta(String routa) {
        this.routa = routa;
    }

    public String getRoute() {
        return route;
    }

    public void setRoute(String route) {
        this.route = route;
    }

    public String getSakes() {
        return sakes;
    }

    public void setSakes(String sakes) {
        this.sakes = sakes;
    }

    public String getSbgrp() {
        return sbgrp;
    }

    public void setSbgrp(String sbgrp) {
        this.sbgrp = sbgrp;
    }

    public String getSdabw() {
        return sdabw;
    }

    public void setSdabw(String sdabw) {
        this.sdabw = sdabw;
    }

    public String getSpaiv() {
        return spaiv;
    }

    public void setSpaiv(String spaiv) {
        this.spaiv = spaiv;
    }

    public String getSpeaccappsts() {
        return speaccappsts;
    }

    public void setSpeaccappsts(String speaccappsts) {
        this.speaccappsts = speaccappsts;
    }

    public String getSpebillingind() {
        return spebillingind;
    }

    public void setSpebillingind(String spebillingind) {
        this.spebillingind = spebillingind;
    }

    public String getSpecarrierind() {
        return specarrierind;
    }

    public void setSpecarrierind(String specarrierind) {
        this.specarrierind = specarrierind;
    }

    public String getSpechngsys() {
        return spechngsys;
    }

    public void setSpechngsys(String spechngsys) {
        this.spechngsys = spechngsys;
    }

    public String getSpegeoroute() {
        return spegeoroute;
    }

    public void setSpegeoroute(String spegeoroute) {
        this.spegeoroute = spegeoroute;
    }

    public String getSpegeorouteind() {
        return spegeorouteind;
    }

    public void setSpegeorouteind(String spegeorouteind) {
        this.spegeorouteind = spegeorouteind;
    }

    public String getSpegtsrel() {
        return spegtsrel;
    }

    public void setSpegtsrel(String spegtsrel) {
        this.spegtsrel = spegtsrel;
    }

    public String getSpegtsrtcde() {
        return spegtsrtcde;
    }

    public void setSpegtsrtcde(String spegtsrtcde) {
        this.spegtsrtcde = spegtsrtcde;
    }

    public String getSpeinvbfrgi() {
        return speinvbfrgi;
    }

    public void setSpeinvbfrgi(String speinvbfrgi) {
        this.speinvbfrgi = speinvbfrgi;
    }

    public String getSpelescenario() {
        return spelescenario;
    }

    public void setSpelescenario(String spelescenario) {
        this.spelescenario = spelescenario;
    }

    public String getSpelifextype() {
        return spelifextype;
    }

    public void setSpelifextype(String spelifextype) {
        this.spelifextype = spelifextype;
    }

    public String getSpelocseq() {
        return spelocseq;
    }

    public void setSpelocseq(String spelocseq) {
        this.spelocseq = spelocseq;
    }

    public String getSpeloekz() {
        return speloekz;
    }

    public void setSpeloekz(String speloekz) {
        this.speloekz = speloekz;
    }

    public String getSpeorigsys() {
        return speorigsys;
    }

    public void setSpeorigsys(String speorigsys) {
        this.speorigsys = speorigsys;
    }

    public String getSpepronumber() {
        return spepronumber;
    }

    public void setSpepronumber(String spepronumber) {
        this.spepronumber = spepronumber;
    }

    public String getSpeqistatus() {
        return speqistatus;
    }

    public void setSpeqistatus(String speqistatus) {
        this.speqistatus = speqistatus;
    }

    public String getSperedind() {
        return speredind;
    }

    public void setSperedind(String speredind) {
        this.speredind = speredind;
    }

    public String getSpereltmstmp() {
        return spereltmstmp;
    }

    public void setSpereltmstmp(String spereltmstmp) {
        this.spereltmstmp = spereltmstmp;
    }

    public String getSperetcanc() {
        return speretcanc;
    }

    public void setSperetcanc(String speretcanc) {
        this.speretcanc = speretcanc;
    }

    public String getSperevvlstk() {
        return sperevvlstk;
    }

    public void setSperevvlstk(String sperevvlstk) {
        this.sperevvlstk = sperevvlstk;
    }

    public String getSpeshpinfsts() {
        return speshpinfsts;
    }

    public void setSpeshpinfsts(String speshpinfsts) {
        this.speshpinfsts = speshpinfsts;
    }

    public String getSpettype() {
        return spettype;
    }

    public void setSpettype(String spettype) {
        this.spettype = spettype;
    }

    public String getSpeunitsystem() {
        return speunitsystem;
    }

    public void setSpeunitsystem(String speunitsystem) {
        this.speunitsystem = speunitsystem;
    }

    public String getSpewauhrist() {
        return spewauhrist;
    }

    public void setSpewauhrist(String spewauhrist) {
        this.spewauhrist = spewauhrist;
    }

    public String getSpewazoneist() {
        return spewazoneist;
    }

    public void setSpewazoneist(String spewazoneist) {
        this.spewazoneist = spewazoneist;
    }

    public String getStafo() {
        return stafo;
    }

    public void setStafo(String stafo) {
        this.stafo = stafo;
    }

    public String getStcur() {
        return stcur;
    }

    public void setStcur(String stcur) {
        this.stcur = stcur;
    }

    public String getStgelocchange() {
        return stgelocchange;
    }

    public void setStgelocchange(String stgelocchange) {
        this.stgelocchange = stgelocchange;
    }

    public String getStwae() {
        return stwae;
    }

    public void setStwae(String stwae) {
        this.stwae = stwae;
    }

    public String getStzkl() {
        return stzkl;
    }

    public void setStzkl(String stzkl) {
        this.stzkl = stzkl;
    }

    public String getStzzu() {
        return stzzu;
    }

    public void setStzzu(String stzzu) {
        this.stzzu = stzzu;
    }

    public String getTcode() {
        return tcode;
    }

    public void setTcode(String tcode) {
        this.tcode = tcode;
    }

    public String getTddat() {
        return tddat;
    }

    public void setTddat(String tddat) {
        this.tddat = tddat;
    }

    public String getTduhr() {
        return tduhr;
    }

    public void setTduhr(String tduhr) {
        this.tduhr = tduhr;
    }

    public String getTernr() {
        return ternr;
    }

    public void setTernr(String ternr) {
        this.ternr = ternr;
    }

    public String getTmctrlkey() {
        return tmctrlkey;
    }

    public void setTmctrlkey(String tmctrlkey) {
        this.tmctrlkey = tmctrlkey;
    }

    public String getTpgrp() {
        return tpgrp;
    }

    public void setTpgrp(String tpgrp) {
        this.tpgrp = tpgrp;
    }

    public String getTpqua() {
        return tpqua;
    }

    public void setTpqua(String tpqua) {
        this.tpqua = tpqua;
    }

    public String getTpsid() {
        return tpsid;
    }

    public void setTpsid(String tpsid) {
        this.tpsid = tpsid;
    }

    public String getTragr() {
        return tragr;
    }

    public void setTragr(String tragr) {
        this.tragr = tragr;
    }

    public String getTraid() {
        return traid;
    }

    public void setTraid(String traid) {
        this.traid = traid;
    }

    public String getTraty() {
        return traty;
    }

    public void setTraty(String traty) {
        this.traty = traty;
    }

    public String getTrmtyp() {
        return trmtyp;
    }

    public void setTrmtyp(String trmtyp) {
        this.trmtyp = trmtyp;
    }

    public String getTrspg() {
        return trspg;
    }

    public void setTrspg(String trspg) {
        this.trspg = trspg;
    }

    public String getTsegfl() {
        return tsegfl;
    }

    public void setTsegfl(String tsegfl) {
        this.tsegfl = tsegfl;
    }

    public String getTsegtp() {
        return tsegtp;
    }

    public void setTsegtp(String tsegtp) {
        this.tsegtp = tsegtp;
    }

    public String getTzonis() {
        return tzonis;
    }

    public void setTzonis(String tzonis) {
        this.tzonis = tzonis;
    }

    public String getTzonrc() {
        return tzonrc;
    }

    public void setTzonrc(String tzonrc) {
        this.tzonrc = tzonrc;
    }

    public String getVbeak() {
        return vbeak;
    }

    public void setVbeak(String vbeak) {
        this.vbeak = vbeak;
    }

    public String getVbund() {
        return vbund;
    }

    public void setVbund(String vbund) {
        this.vbund = vbund;
    }

    public String getVerur() {
        return verur;
    }

    public void setVerur(String verur) {
        this.verur = verur;
    }

    public String getVerursys() {
        return verursys;
    }

    public void setVerursys(String verursys) {
        this.verursys = verursys;
    }

    public String getVganz() {
        return vganz;
    }

    public void setVganz(String vganz) {
        this.vganz = vganz;
    }

    public String getVkbur() {
        return vkbur;
    }

    public void setVkbur(String vkbur) {
        this.vkbur = vkbur;
    }

    public String getVkoiv() {
        return vkoiv;
    }

    public void setVkoiv(String vkoiv) {
        this.vkoiv = vkoiv;
    }

    public String getVkorg() {
        return vkorg;
    }

    public void setVkorg(String vkorg) {
        this.vkorg = vkorg;
    }

    public String getVlstk() {
        return vlstk;
    }

    public void setVlstk(String vlstk) {
        this.vlstk = vlstk;
    }

    public String getVoleh() {
        return voleh;
    }

    public void setVoleh(String voleh) {
        this.voleh = voleh;
    }

    public String getVolum() {
        return volum;
    }

    public void setVolum(String volum) {
        this.volum = volum;
    }

    public String getVsart() {
        return vsart;
    }

    public void setVsart(String vsart) {
        this.vsart = vsart;
    }

    public String getVsbed() {
        return vsbed;
    }

    public void setVsbed(String vsbed) {
        this.vsbed = vsbed;
    }

    public String getVtwiv() {
        return vtwiv;
    }

    public void setVtwiv(String vtwiv) {
        this.vtwiv = vtwiv;
    }

    public String getWadat() {
        return wadat;
    }

    public void setWadat(String wadat) {
        this.wadat = wadat;
    }

    public String getWaerk() {
        return waerk;
    }

    public void setWaerk(String waerk) {
        this.waerk = waerk;
    }

    public String getWauhr() {
        return wauhr;
    }

    public void setWauhr(String wauhr) {
        this.wauhr = wauhr;
    }

    public String getXabln() {
        return xabln;
    }

    public void setXabln(String xabln) {
        this.xabln = xabln;
    }

    public String getXblnr() {
        return xblnr;
    }

    public void setXblnr(String xblnr) {
        this.xblnr = xblnr;
    }

    public String getXwoff() {
        return xwoff;
    }

    public void setXwoff(String xwoff) {
        this.xwoff = xwoff;
    }

    public String getZukrl() {
        return zukrl;
    }

    public void setZukrl(String zukrl) {
        this.zukrl = zukrl;
    }

    public String getVbeln() {
        return this.vbeln;
    }

    public void setVbeln(String vbeln) {
        this.vbeln = vbeln;
    }

    public String getVstel() {
        return this.vstel;
    }

    public void setVstel(String vstel) {
        this.vstel = vstel;
    }

    public String getGstrp() {
        return this.gstrp;
    }

    public void setGstrp(String gstrp) {
        this.gstrp = gstrp;
    }

    public String getLfart() {
        return this.lfart;
    }

    public void setLfart(String lfart) {
        this.lfart = lfart;
    }

    public String getVbtyp() {
        return this.vbtyp;
    }

    public void setVbtyp(String vbtyp) {
        this.vbtyp = vbtyp;
    }

    public String getLfdat() {
        return this.lfdat;
    }

    public void setLfdat(String lfdat) {
        this.lfdat = lfdat;
    }

    public String getErdat() {
        return this.erdat;
    }

    public void setErdat(String erdat) {
        this.erdat = erdat;
    }

    public String getBolnr() {
        return this.bolnr;
    }

    public void setBolnr(String bolnr) {
        this.bolnr = bolnr;
    }

    public String getLifex() {
        return this.lifex;
    }

    public void setLifex(String lifex) {this.lifex = lifex; }

    public String getWadatist() {
        return this.wadatist;
    }

    public void setWadatist(String wadatist) {
        this.wadatist = wadatist;
    }

    public String getLifnr() {
        return this.lifnr;
    }

    public void setLifnr(String lifnr) {
        this.lifnr = lifnr;
    }

    public String getWerks() {
        return this.werks;
    }

    public void setWerks(String werks) {
        this.werks = werks;
    }


}
