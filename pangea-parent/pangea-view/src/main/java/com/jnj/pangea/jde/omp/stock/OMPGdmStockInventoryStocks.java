...
public class OMPGdmStockInventoryStocks implements IEventProcessor {
		...
		if (inventoryStockList != null && inventoryStockList.size() > 0) {
			for (Map.Entry<String, String> inventoryStockListEntry : inventoryStockList) {
				...
				if (StringInner.isStringNotEmpty(localMaterial)
					...
					Map map1 = getMaterialGlobalV1(sourceSystem, localMaterial);
					if (map1 != null) {
						primaryPlanningCode = String.valueOf(map1
								.get("primaryPlanningCode"));
						materialMumber = String.valueOf(map1
								.get("materialMumber"));
						...
						if (localConsignmentSpecialStockIndicator.equals("K")
							...
							if (StringInner
								...
								if (StringInner.isStringNotEmpty(sourceSystem)
									...
									Map map2 = getCnsSplPlnLoc(sourceSystem,
											localVendorNumber, vendorOrCustomer);
									if (map2 != null) {
				...
				if (StringInner.isStringEmpty(localSpecialStockIndicator)) {
					...
					if (map3 != null) {
						...
						if (localPlanningRelevant.equals("X")) {
							locationId = StringInner.join(sourceSystem, "_",
									localPlant);
						}
					}
				}
					...
					if (localSpecialStockIndicator.equals("O")) {
						Map map4 = getCnsSplPlnLocWithLocalPlant(sourceSystem,
								localNumber, localPlant, vendorOrCustomer);
						if (map4 != null) {
							localNumber = String.valueOf(map4
									.get("localNumber"));
				...
				if (StringInner.isStringNotEmpty(localMaterial)
					...
					Map map5 = getCnsMaterialPlanStatus(localMaterial,
							localPlant, sourceSystem);
					if (map5 != null) {
						spRelevant = String.valueOf(map5.get("spRelevant"));
						noPlanRelevant = String.valueOf(map5
								.get("noPlanRelevant"));
						if (spRelevant.equals("X")
								|| noPlanRelevant.equals("X")) {
							Map map6 = getMaterialGlobalV1(sourceSystem,
									localMaterial);
							if (map6 != null) {
								primaryPlanningCode = String.valueOf(map6
										.get("primaryPlanningCode"));
								materialMumber = String.valueOf(map6
										.get("materialMumber"));
								if (StringInner
	...
	public List getInventoryStock(String sourceSystem, String localPlant,
		...
		List<Map.Entry<String, String>> retList = AdfViewHelper.queryForList(
				"/edm/inventory_stock", queryStr, -1);
