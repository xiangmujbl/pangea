...
public class EMSEdmUnitInputDaoImpl extends CommonDaoImpl {
    ...
    public EMSEdmUnitInputEntity queryEdmUnitInput(String sourceSystem,String mdmSapCode) {
        if(StringUtils.isNotEmpty(sourceSystem)&&StringUtils.isNotEmpty(mdmSapCode)){
            String countryQueryString = QueryHelper.buildCriteria(IConstant.EMS_EDM_UNIT_INPUT.SOURCE_SYSTEM)
                    .is(sourceSystem).and(IConstant.EMS_EDM_UNIT_INPUT.LOCAL_UOM).is(mdmSapCode).toQueryString();
            EMSEdmUnitInputEntity emsEdmUnitInputEntity = queryForObject(IConstant.REGION.EMS_EDM_UNIT_INPUT, countryQueryString, EMSEdmUnitInputEntity.class);
