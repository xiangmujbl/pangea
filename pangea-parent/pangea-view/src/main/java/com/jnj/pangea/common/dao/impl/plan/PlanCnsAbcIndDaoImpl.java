...
public class PlanCnsAbcIndDaoImpl extends CommonDaoImpl {
    ...
    public PlanCnsAbcIndEntity getEntityWithLocalIndicatorAndsourceSystem(String localAbcIndicator, String sourceSystem) {
        String queryString = QueryHelper.buildCriteria(IConstant.PLAN_CNS_ABC_IND.LOCAL_ABC_INDICATOR).is(localAbcIndicator).and(IConstant.PLAN_CNS_ABC_IND.SOURCE_SYSTEM).is(sourceSystem).toQueryString();
