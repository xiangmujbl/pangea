		...
		if (null != countryV1Entity) {
			...
			if ((countryV1Entity.getCountryCode() != null
				...
				if (StringUtils.isNotEmpty(sourceSystem)) {
					...
					List<PlanCnsPlanParameterEntity> planCnsPlanParameterEntityList = planCnsPlanParameterDao
							.getEntitiesWithSourceSystem(sourceSystem);
			...
			// rules J1 T2
			if (StringUtils.isNotEmpty(sourceSystem)) {
				PlanEdmCountryInputEntity edmCountryInputEntity = EDMCountryInputDao
						.getEntityWithFormNameAndLocalCountry(countryV1Entity.getSourceSystem(),
								countryV1Entity.getLocalCountry());

				if (null == edmCountryInputEntity) {
					FailData failData = writeFailDataToRegion(countryV1Entity, "J1",
							"localcountry do not exist in edm country");
					resultObject.setFailData(failData);
					return resultObject;
				}
				EDMCurrencyV1Entity edmCurrencyV1Entity = EDMCurrencyV1Dao.getEntityWithLocalCurrencyAndSourceSystem(
						edmCountryInputEntity.getLocalCurrency(), sourceSystem);

				if (null == edmCurrencyV1Entity) {

					FailData failData = writeFailDataToRegion(countryV1Entity, "J1",
							"localCurrecny do not exist in edm currency");
					resultObject.setFailData(failData);
					return resultObject;
				}
